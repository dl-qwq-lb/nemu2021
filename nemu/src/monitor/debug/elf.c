...
int get_str_addr(char* args, bool *success){
	int i;
	char* temp_str = strtab;
	for(i = 0; i < nr_symtab_entry; i++){
		if(strcmp(args, temp_str) == 0){
			break;
		}
		//Log("%s\n", temp_str);
	...
	//Log("%d\n", i);
	int non_object;
	for(non_object = 0; non_object < nr_symtab_entry; non_object ++){
		//Log("%c", symtab[non_object].st_info + 48);
		if(symtab[non_object].st_info == 4){
			break;
		}
	}
	//Log("%d\n", i + non_object);
	int indx = i + non_object;
	if(symtab[indx].st_info != 17){
		*success = false;
		return 0;
	} 
	*success = true;
	return symtab[indx].st_value;
}
