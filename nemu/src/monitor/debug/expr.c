...
extern int get_str_addr(char* args, bool *success);

enum {
	NOTYPE = 256, EQ

	/* TODO: Add more token types */
        , NUM, NEQ, OR, AND, REG, REF, NEG, IDEN
};
			...
			if(regexec(&re[i], e + position, 1, &pmatch, 0) == 0 && pmatch.rm_so == 0) {
				...
				switch(rules[i].token_type) {
					...
					//default: panic("please implement me");
										...
										case IDEN: sprintf(tokens[nr_token].str, "%.*s", substr_len, substr_start);
...
static int find_dominated_op(int s, int e, bool *success) {
	...
	for(i = s; i <= e; i ++) {
		switch(tokens[i].type) {
			case REG: case NUM: case IDEN: break;
				...
				if(bracket_level == 0) {
					if(dominated_op == -1 || 
							...
							(op_prec_cmp(tokens[dominated_op].type, tokens[i].type) == 0 && 
							 ...
							 tokens[i].type != NEG && tokens[i].type != REF && tokens[i].type != IDEN) ) {
						dominated_op = i;
					}
				}
				break;
	...
	if(s > e) {
		...
		*success = false;
		...
		switch(tokens[s].type) {
			...
			case IDEN: {
				val = get_str_addr(tokens[s].str, success);break;
			}
...
uint32_t expr(char *e, bool *success) {
	...
	for(i = 0; i < nr_token; i ++) {
		if(tokens[i].type == '-') {
			...
			if( !(prev_type == ')' || prev_type == NUM || prev_type == REG || prev_type == IDEN) ) {
				tokens[i].type = NEG;
			}
		}
